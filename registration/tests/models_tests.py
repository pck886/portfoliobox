...
from registration.models import RegistrationProfile, get_or_none
...
class RegistrationModelTests(TestCase):
    ...
    def test_activation_email(self):
        ...
        new_user = UserModel().objects.create_user(**self.user_info)
        profile = RegistrationProfile.objects.create_profile(new_user)
        profile.send_activation_email(Site.objects.get_current(), request)
    ...
    def test_activate_get_profile_True(self):
        user = RegistrationProfile.objects.create_inactive_user(
            site=Site.objects.get_current(), send_email=False, **self.user_info)

        profile = RegistrationProfile.objects.get(user=user)
        activated = (RegistrationProfile.objects
                     .activate_user(profile.activation_key, True))

        self.assertEqual(activated, profile)
    ...
    def test_get_or_none(self):
        ...
        one_user = get_or_none(User, username=user.username)
        mutiple_user = get_or_none(User, email='alice@example.com')

        self.assertEqual(user.username, one_user.username)
        self.assertEqual('bob', mutiple_user.username)
        self.assertRaises(Exception, get_or_none(User, username=None))
